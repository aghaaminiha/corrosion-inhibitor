...
param = dict(test_size=0.25, cv=5, scoring='mse', iterations=20,
             ...
             production=True, sensitivity=False)
...
# # pre-processing data
dataCleaned, off_replicas = remove_replicas(dataAll)
dataSelected = select_features(dataCleaned)
...
if param['production']:
    ...
    for seatOut in seatOuts:
        ...
        for exp in seatOut:
            ...
            production_plot(dataAll, dataCleaned, y_pred, mse_prod, f'testingTheModel/{seatOut}', 'Log', exp, seatOut)
            production_plot(dataAll, dataCleaned, y_pred, mse_prod, f'testingTheModel/{seatOut}', 'Norm', exp, seatOut)
...
if param['sensitivity']:
    ...
    for experiment in experiments:
        ...
        best_reg.fit(X_train, y_train)
        testing_sens, time_sens = sensitivity(dataCleaned, testing_sens, experiment)
        for key in features_reg:
            ...
            for value in features_reg[key][0]:
                ...
                if first and key in ['CI', 'pH', 'Brine_Type']:
                    ...
                    key_mean, key_std = np.mean(dataCleaned[key]), np.std(dataCleaned[key])
